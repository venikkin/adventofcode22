...
determine whether the sprite is visible the instant each pixel is drawn. If the sprite is positioned such that
one of its three pixels is the pixel currently being drawn, the screen produces a lit pixel (#); otherwise, the screen leaves the pixel dark (.).
...
func main() {
	...
	register := 1
	crt := ""
	for _, line := range utils.ReadLines("19.txt") {
		trimmed := strings.TrimSpace(line)
		if trimmed[:4] == "addx" {
			regInc := utils.AtoiOrPanic(trimmed[5:])

			printToCrt(&crt, cycle, register)
			cycle++
...
func printToCrt(crt *string, cycle, register int) {
	cycleMod := cycle % 40
	if cycleMod >= register-1 && cycleMod <= register+1 {
		*crt = *crt + "X"
	} else {
		*crt = *crt + "."
	}
