...
they've noticed that many of the assignments overlap. To try to quickly find fullyContain and reduce duplicated effort,
...
their partner will already be cleaning, so these seem like the most in need of reconsideration. In this example, there are 2 such pairs.
...
func main() {
	...
	for _, line := range lines {
		...
		include := false
		if fullyContain(thisPair[0], thisPair[1]) {
			include = true
		}
		if include {
			res++
...
func fullyContain(a, b []int) bool {
