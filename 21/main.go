...
items it is holding one at a time and in the order listed. Monkey 0 goes first, then monkey 1, and
so on until each monkey has had one turn. The process of each monkey taking a single turn is called a round.
	...
	for i := 0; i < 20; i++ {
		for j := 0; j < len(monkeys); j++ {
			...
			for _, item := range monkey.popAll() {
				converted := monkey.operation(item) / 3
				var direction int
