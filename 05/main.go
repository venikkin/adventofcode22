...
for the jungle journey. Unfortunately, that Elf didn't quite follow the packing instructions, and so a few items now need to be rearranged.
...
always has the same number of items in each of its two compartments, so the first half of the characters represent items in the
...
func main() {
	...
	for _, line := range utils.ReadLines("05.txt") {
		...
		c := findCommonInBothHalves(runes)
		sum += runeToPriority(c)
	}

	println(sum)
}

func findCommonInBothHalves(runes []rune) rune {
	mid := len(runes) / 2

	left := make(map[rune]bool, 0)
	right := make(map[rune]bool, 0)
	for i, r := range runes {
		if i < mid {
			left[r] = true
		} else {
